...
public class VideoScan {
	...
	private void createGif() throws IOException {
		...
		statusProcessor.appendStatus(String.format("Testing Size: %dx%d%n", newWidth, newHeight));
		
		statusProcessor.appendStatus("Scaling Video...");

		String ffmpeg = FFmpegManager.getFFmpegManager().getFFmpegLocation();
		
		MainFrame.exec(true, ffmpeg, "-y", "-ss", Double.toString(this.startTime), "-i", location, "-map", "0:v", "-t", Double.toString(this.endTime - this.startTime), "-pix_fmt", "yuv420p", "-s", newWidth + "x" + newHeight, halveFramerate ? "-r" : "-y", halveFramerate ? String.format("%f", framerate * 0.5D) : "-y", "-c", "ffv1", "-f", "matroska", this.mkvFile.getAbsolutePath());
		
		statusProcessor.replaceStatus("Scaling Video... Done.");
		statusProcessor.appendStatus("Generating Palette... ");
		...
		statusProcessor.replaceStatus("Generating Palette... Done.");
		statusProcessor.appendStatus("Generating GIF... ");
		
		MainFrame.exec(true, ffmpeg, "-y", "-i", this.mkvFile.getAbsolutePath(), "-i", this.paletteFile.getAbsolutePath(), "-lavfi", "paletteuse", "-c", "gif", "-f", "gif", this.gifFile.getAbsolutePath());
		
		statusProcessor.replaceStatus(String.format("Generating GIF... Done.%n"));
	}
